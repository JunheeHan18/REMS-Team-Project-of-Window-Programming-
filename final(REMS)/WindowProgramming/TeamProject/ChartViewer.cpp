...
void CChartViewer::UpdateElement()
{
	double idx[4] = { 0, };
	for (int i = 0; i < CTabCheckRequest::ArrayCnt; i++) {
		if (CTabCheckRequest::typeCount[i] > 0) {
			idx[i] = CTabCheckRequest::typeCount[i];
		}
	}
	//차트로 보여줄 데이터 지정
	double data[sizeof(idx) / sizeof(idx[0])];
	for (int i = 0; i < sizeof(idx) / sizeof(idx[0]); i++) {
		data[i] = idx[i];
	}

	//각 데이터에 대한 레이블 지정
	const char* labels[] = { "VS232454AA","MSS4052220W", "DS-20E5174-A", "DS-20E4562-AE" };


	//각 섹터에 대한 깊이 지정
	/*int size = (sizeof(idx) / sizeof(double));
	...
	double depths[sizeof(idx) / sizeof(idx[0])];
	for (int i = 0; i < sizeof(idx) / sizeof(idx[0]); i++) {
		depths[i] = idx[i]*1.7;
	}

	//차트 생성 (차트 사이즈 가로, 세로, 배경색, ?, ?)
	PieChart *c = new PieChart(800, 600, 0xddddff, -1, 1);

	//파이 사이즈 지정(중심좌표 x, y, 반지름)
	c->setPieSize(420,300,300);

	//타이틀 지정 (문자열, 폰트, 크기), 배경색 지정(배경색)
	c->addTitle("Rate of Check Request", "calibri.ttf", 14)->setBackground(0xaaaaff);

	//데이터와 레이블 정보를 차트에 지정
	c->setData(DoubleArray(data, (int)(sizeof(data) / sizeof(data[0]))),
		StringArray(labels, (int)(sizeof(labels) / sizeof(labels[0]))));

	//파이 차트를 깊이 정보와 함꼐 3D로 그려지도록 설정
	c->set3D(DoubleArray(depths, (int)(sizeof(depths) / sizeof(depths[0]))));

	//파이 차트에서 데이터 구분을 시작하는 경계 각도 설정
	c->setStartAngle(450);
	//차트를 이미지로 저장
